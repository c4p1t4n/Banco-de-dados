...
alter table Atleta add  fkPais int,add foreign key(fkPais) references pais(idPais);
select * from Atleta;
desc Atleta;
insert into pais values(101,'EUA','Washington'),
					   (102,'Italia','Roma');
update Atleta set fkPais=101 where id_atleta in (1,5,6);
update Atleta set fkPais=102 where id_atleta in (2,3,4);
select * from Atleta,pais where fkPais = idPais;
select * from Atleta,pais where fkPais = 101 and idPais = 101;
